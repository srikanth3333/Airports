  ...
  renderItem({ item, index }) {
    ...
    return (
      <DropdownItem index={index} {...props}>
        <Text
          style={
            ([styles.item, itemTextStyle, textStyle, { color: "red" }])
          }
          numberOfLines={1}
        >
          {title}
    ...
    return (
      ...
      >
        <TouchableWithoutFeedback {...touchableProps}>
          <ContainerView pointerEvents="box-only">
            {this.renderBase(props)}
            <IconStyle
              source={downArrow}
              style={{ transform: [{ rotate: modal ? "180deg" : "0deg" }] }}
            />
          </ContainerView>
        </TouchableWithoutFeedback>
          ...
          >
            <FlatListContainer
              style={[
                ...
                { marginTop: 75, marginStart: I18nManager.isRTL ? 14 : 0 },
              ]}
              onStartShouldSetResponder={() => true}
            >
              <FlatList
