...
import { MADropDownStyles } from "./styles";
...
export default class Dropdown extends PureComponent {
  ...
  onPress(event) {
    ...
    this.container.measureInWindow((x, y, containerWidth, containerHeight) => {
      ...
      if (Platform.OS === "android" || iosPickerDisabled) {
        this.setState({
          modal: true,
          width: right - left,
          top,
        ...
        onIosPickerPressed &&
          onIosPickerPressed(this.state.isClicked, Number(selected));
      }
  ...
  renderBase(props) {
    ...
    return (
      <MADropDownStyles.DropDownInput
      isFromOrders={this.props.isFromOrders}
        ...
        //     : light.black
        // }
        value={title ? title : value} //Dharmaraj Updated title to Value for fix the ABMA-2990 issue
        editable={false}
  ...
  renderAccessory() {
    ...
    return (
      <View style={MADropDownStyles.styles.accessory}>
        <View style={MADropDownStyles.styles.triangleContainer}>
          <View style={[MADropDownStyles.styles.triangle, triangleStyle]} />
        </View>
      </View>
    );
  }
  ...
  render() {
    ...
    let { left, top, width, opacity, selected, modal, isClicked } = this.state;
    ...
    let pickerEnabled =
      Platform.OS === "ios" && isPickerEnabled !== undefined
        ? isPickerEnabled
        : isClicked;

    return (
      <MADropDownStyles.MainContainer
        onLayout={this.onLayout}
        ref={this.updateContainerRef}
        style={[containerStyle]}
        isFromContactUs={isFromContactUs}
      >
        <TouchableWithoutFeedback {...touchableProps}>
          <MADropDownStyles.ContainerView pointerEvents="box-only">
            {this.renderBase(props)}
            <MADropDownStyles.IconStyle
              source={downArrow}
              style={{
                left: 7,
                transform: [{ rotate: modal ? "180deg" : "0deg" }],
              }}
            />
          </MADropDownStyles.ContainerView>
        </TouchableWithoutFeedback>

        <Modal
          visible={disabled ? false : modal}
          transparent={true}
        ...
        >
          <Animated.View
            style={[
              MADropDownStyles.styles.overlay,
              overlayStyle,
          ...
          >
            <MADropDownStyles.FlatListContainer
              style={[
                pickerStyle,
                pickerStyleOverrides,
                isFromContactUs ? { marginTop: 75, marginStart: I18nManager.isRTL ? 14 : 15, width: '87.5%' } : {marginTop: 20, marginStart: I18nManager.isRTL ? 14 : 8,  width: '90.5%'},
            ...
            >
              <FlatList
                ...
                data={data}
                style={MADropDownStyles.styles.scroll}
                renderItem={this.renderItem}
                keyExtractor={this.keyExtractor}
                scrollEnabled={visibleItemCount < itemCount}
                contentContainerStyle={MADropDownStyles.styles.scrollContainer}
              />
            </MADropDownStyles.FlatListContainer>
          </Animated.View>
        </Modal>
